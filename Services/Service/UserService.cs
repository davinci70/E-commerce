    ...
    ApplicationDbContext context) : IUserService
{
    ...
    public async Task<Result<UserResponse>> AddAsync(CreateUserRequest request, CancellationToken cancellationToken)
    {
        ...
        var IsPhoneNumberExists = await _userManager.Users.AnyAsync(x => x.PhoneNumber == request.PhoneNumber, cancellationToken);

        if (IsPhoneNumberExists)
            return Result.Failure<UserResponse>(UserErrors.DublicatedPhoneNumber);
    ...
    public async Task<Result> UpdateAsync(string id, UpdateUserRequest request, CancellationToken cancellationToken)
    {
        ...
        var IsPhoneNumberExists = await _userManager.Users.AnyAsync(x => x.PhoneNumber == request.PhoneNumber && x.Id != id, cancellationToken);
